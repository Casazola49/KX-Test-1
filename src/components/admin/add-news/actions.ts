...
const NewsActionSchema = z.object({
  ...
  imageUrl: z.string().url({ message: 'La URL de la imagen es obligatoria.' }), // No optional, always required
...
export async function saveNews(data: z.infer<typeof NewsActionSchema>) {
  try {
    ...
    const { id, imageUrl, galleryImageUrls, isMain, ...newsData } = validatedData;
